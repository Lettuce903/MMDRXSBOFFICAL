...
#Child nodes
onready var cult_animation = $SpriteMain/Sprite/CultAnimation
...
func _process(delta: float) -> void:
	if player != null:
		...
		if within_player_range(attack_range) && is_attack_ready:
			if randi() % 3 + 1 == 1:
				cult_animation.play("Jump")
				platformer_behavior.jump_start()
			else:
				cult_animation.play("Shoot")
...
func _on_PlatformerBehavior_landed() -> void:
	if cult_animation.current_animation == "Jump":
		standby()
